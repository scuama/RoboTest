...
"""
遮挡实验主运行脚本 - 带动作序列缓存版本
与 pi05_libero_visual_inference_fixed.py 保持一致的推理策略
"""
...
def run_group_with_cache(env, config: Dict, group_config: Dict, group_name: str, 
    ...
    for episode_idx in range(episodes_per_group):
        ...
        # 应用遮挡（如果需要）
        # ✅ 支持两种模式：
        # 1. use_bddl_stacking=True: BDDL文件中已定义堆叠，不需要SceneModifier
        # 2. use_bddl_stacking=False: 使用SceneModifier动态修改场景
        use_bddl_stacking = group_config.get('use_bddl_stacking', False)
        
        if group_config.get('use_obstruction', False) and not use_bddl_stacking:
            # 使用SceneModifier动态修改场景（原有逻辑）
            modifier = SceneModifier(env, config)
            success = modifier.apply_obstruction()
            if not success:
                print(f"  ⚠️  遮挡应用失败")
                continue
        elif use_bddl_stacking:
            # BDDL中已定义堆叠，无需额外修改
            print(f"  ℹ️  使用BDDL定义的堆叠场景（无需SceneModifier）")
